...
namespace Nhom11_Quanlybangiay.NhapHang
{
    public partial class frmNhapHang : Form
    {
        ...
        private void button5_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Danh sách không có mặt hàng nào !!!", "Lưu ý", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
            else
            {
                DialogResult kq = MessageBox.Show("Vui lòng kiểm tra kĩ thông tin !!!", "Lưu ý", MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
                if (kq.Equals(DialogResult.Yes))
                {
                    data.themhoadonnhap(cbboxncc.SelectedValue.ToString(), manql, dtpNgayNhap.Value);
                    txtsohd.Text = data.xemhoadonvuamoinhap().Rows[0][0].ToString();
                    for (int i = 0; i < dgvdanhsachhang.Rows.Count; i++)
                    {
                        //themsanphamvaohoadon(int masp, int sohdn,string size,int dongia,int soluongnhap,int thanhtien)
                        int masp = Convert.ToInt32(dgvdanhsachhang.Rows[i].Cells[0].Value.ToString());
                        int sohdn = Convert.ToInt32(txtsohd.Text);
                        string size = dgvdanhsachhang.Rows[i].Cells[2].Value.ToString();
                        int dongia = Convert.ToInt32(dgvdanhsachhang.Rows[i].Cells[3].Value.ToString());
                        int slnhap = Convert.ToInt32(dgvdanhsachhang.Rows[i].Cells[4].Value.ToString());
                        int thanhtien = Convert.ToInt32(dgvdanhsachhang.Rows[i].Cells[5].Value.ToString());
                        data.themsanphamvaohoadon(masp, sohdn, size, slnhap, dongia, thanhtien);
                    }
                    ...
                    for (int i = 0; i < dgvdanhsachhang.Rows.Count; i++)
                    {
                        ...
                        int slnhap = Convert.ToInt32(dgvdanhsachhang.Rows[i].Cells[4].Value.ToString());
                        con.Open();
                        string sql = "Select * from SLCON where Masp='" + masp + "' and size='" + size + "'   ";
                        SqlCommand cmd = new SqlCommand(sql, con);
                        SqlDataReader dt = cmd.ExecuteReader();

                        if(dt.Read()==true)
                        { // NẾU MẶT HÀNG ĐÃ TỒN TẠI THÌ CẬP NHẬT THÊM SỐ LƯỢNG 
                            con.Close(); // ĐÓNG CONNECT CỦA READER
                            con.Open();
                            string capnhat = "exec CapNhatSoLuongHangXoa '"+masp+"','"+size+"','"+slnhap+"'    ";
                            SqlCommand cmd_capnhat = new SqlCommand(capnhat, con);
                            cmd_capnhat.ExecuteNonQuery();
                            con.Close();
                        }    
                        else
                        {// NẾU MẶT HÀNG CHƯA TỒN TẠI THÌ CHÈN MỚI DỮ LIỆU VÀO BẢNG
                            con.Close();
                            con.Open();
                            string insert = " Insert into slcon values ('"+masp+"','"+size+"','"+slnhap+"')    ";
                            SqlCommand cmd_inset = new SqlCommand(insert, con);
                    ...
                    frmChitietnhaphang f = new frmChitietnhaphang(Convert.ToInt32(txtsohd.Text));
                    this.Hide();
                    f.ShowDialog();
                    this.Close();
